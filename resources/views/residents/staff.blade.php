...
<div class="container-xxl flex-grow-1 container-p-y">
    ...
    {{-- Add/Edit Staff Modal --}}
        <x-ui.modal id="staffModal" :title="__('Staff Details')" size="md" scrollable="false" class="staff-modal">
            <x-slot name="slot">
                <form id="staffForm">
                    <input type="hidden" id="staff_id" name="staff_id">
                    <div class="row">
                        <div class="col-md-12 mb-3">
                            <label for="staff_name_en" class="form-label">{{ __('Name (English)') }}</label>
                            <input type="text" class="form-control" id="staff_name_en" name="name_en" required>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="staff_name_ar" class="form-label">{{ __('Name (Arabic)') }}</label>
                            <input type="text" class="form-control" id="staff_name_ar" name="name_ar" required>
                        </div>
                        <div class="col-md-12 mb-3">
                        ...
                        <div class="col-md-12 mb-3">
                            <label for="staff_national_id" class="form-label">{{ __('National ID') }}</label>
                            <input type="text" class="form-control" id="staff_national_id" name="national_id">
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="staff_gender" class="form-label">{{ __('Gender') }}</label>
                            <select class="form-control" id="staff_gender" name="gender">
                        ...
                        <div class="col-md-12 mb-3">
                            <label for="unit_type" class="form-label">{{ __('Unit Type') }}</label>
                            <select class="form-control" id="unit_type" name="unit_type">
                                ...
                                <option value="campus">{{ __('Campus Unit') }}</option>
                            </select>
                        </div>
                        <div class="col-md-12 mb-3" id="unit_field_container" style="display:none;">
                            <label for="staff_unit_id" class="form-label">{{ __('Unit') }}</label>
                            <select class="form-control" id="staff_unit_id" name="unit_id"></select>
                        </div>
                        <div class="col-md-12 mb-3">
                            <label for="staff_notes" class="form-label">{{ __('Notes') }}</label>
                            <textarea id="staff_notes" name="notes" class="form-control" rows="3"></textarea>
                        </div>
                    </div>
                </form>
            </x-slot>
            <x-slot name="footer">
                ...
                <button type="submit" class="btn btn-primary" id="saveStaffBtn" form="staffForm">
                    {{ __('Save') }}
                </button>
            </x-slot>
        </x-ui.modal>
...
var TRANSLATION = {
    ...
    labels: {
        ...
        administrative: @json(__('Administrative')),
...
var SelectManager = {
    populateModalTypes: function() {
        ...
        [
            ...
            { id: 'administrative', name: TRANSLATION.labels.administrative, type: 'administrative' },
        ...
        return promise.done(function(response) {
            if (response.success) {
                Utils.populateSelect($select, response.data, { 
                    valueField: 'id', 
                    textField: 'name', 
                    placeholder: TRANSLATION.placeholders.selectUnit, 
                    triggerChange: true
                });
            }
        }).fail(function(xhr) {
            Utils.populateSelect($select, [], { placeholder: TRANSLATION.placeholders.all });
            Utils.handleAjaxError(xhr, xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : null);
        });
    },

    populateSearchDepartments: function() {
        var $select = $('#search_department');
        Utils.populateSelect($select, [], { placeholder: TRANSLATION.placeholders.allDepartments });
            ...
            .done(function(response) {
                if (response.success) {
                    Utils.populateSelect($select, response.data, { 
                        valueField: 'id', 
                        textField: 'name', 
                        placeholder: TRANSLATION.placeholders.allDepartments 
                    });
                }
            })
            .fail(function(xhr) {
    ...
    populateSearchTypes: function() {
        ...
        $select.empty().append('<option value="">' + TRANSLATION.placeholders.allTypes + '</option>');
        [
            { id: 'faculty', name: TRANSLATION.labels.faculty },
            { id: 'administrative', name: TRANSLATION.labels.administrative },
    ...
    populateSearchFaculties: function() {
        ...
        Utils.populateSelect($select, [], { placeholder: TRANSLATION.placeholders.allFaculties });
            ...
            .done(function(response) {
                if (response.success) {
                    Utils.populateSelect($select, response.data, { 
                        valueField: 'id', 
                        textField: 'name', 
                        placeholder: TRANSLATION.placeholders.allFaculties 
                    });
                }
            })
            .fail(function(xhr) {
...
var Select2Manager = {
    config: {
        ...
        modal: {
            '#staff_gender': { placeholder: TRANSLATION.placeholders.selectGender, dropdownParent: $('#staffModal') },
            '#unit_type': { placeholder: TRANSLATION.placeholders.selectType, dropdownParent: $('#staffModal') },
            '#staff_unit_id': { placeholder: TRANSLATION.placeholders.selectUnit, dropdownParent: $('#staffModal') }
        }
    },
    ...
    initSearchSelect2: function() {
        ...
        Object.keys(this.config.search).forEach(function(selector) {
            if (typeof Utils.initSelect2 === 'function') {
                Utils.initSelect2(selector, self.config.search[selector]);
            }
        });
    },
    ...
    initModalSelect2: function() {
        ...
        Object.keys(this.config.modal).forEach(function(selector) {
            if (typeof Utils.initSelect2 === 'function') {
                Utils.initSelect2(selector, self.config.modal[selector]);
            }
        });
    },
...
var StaffManager = {
    ...
    handleTypeChange: function() {
        $(document).on('change', '#unit_type', function() {
            ...
            if (value && (value === 'administrative' || value === 'faculty' || value === 'campus')) {
                ...
                switch (value) {
                    case 'faculty':
                        ...
                        break;
                    case 'administrative':
                        $unitLabel.text(TRANSLATION.labels.administrative);
                        break;
                    case 'campus':
            ...
            ApiService.fetchStaff(staffId)
                .done(function(response) {
                    if (response.success && response.data) {
                        ...
                        $('#staff_gender').val(staff.gender || '').trigger('change');
                        $('#unit_type').val(staff.unit_type || '').trigger('change');
                        
                        // Handle unit field population
                        var dataType = $('#unit_type').find('option:selected').data('type') || '';
                        if (dataType && staff.unit && staff.unit.id) {
                            SelectManager.populateUnitField(dataType).done(function() {
                                $('#staff_unit_id').val(staff.unit.id).trigger('change');
                            });
                        }
        ...
        $(document).on('click', '.viewStaffBtn', function() {
            ...
            ApiService.fetchStaff(staffId)
                .done(function(response) {
                    if (response.success) {
                        var staff = response.data;
                        var unitName = staff.unit && staff.unit.name ? staff.unit.name : TRANSLATION.general.na;
                        
                        $('#view-staff-staff-id').text(staff.id != null ? staff.id : TRANSLATION.general.na);
                        $('#view-staff-name').text(staff.name_en != null ? staff.name_en : TRANSLATION.general.na);
                        $('#view-staff-email').text(staff.email != null ? staff.email : TRANSLATION.general.na);
                        $('#view-staff-national-id').text(staff.national_id != null ? staff.national_id : TRANSLATION.general.na);
                        $('#view-staff-gender').text(staff.gender != null ? staff.gender : TRANSLATION.general.na);
                        $('#view-staff-unit-type').text(staff.unit_type ? staff.unit_type.charAt(0).toUpperCase() + staff.unit_type.slice(1) : TRANSLATION.general.na);
                        $('#view-staff-unit-name').text(unitName);
                        $('#view-staff-notes').text(staff.notes !== null && staff.notes !== undefined ? staff.notes : TRANSLATION.general.na);
                        $('#view-staff-created').text(staff.created_at ? new Date(staff.created_at).toLocaleString() : TRANSLATION.general.na);
    ...
    handleFormSubmit: function() {
        ...
        $('#staffForm').on('submit', function(e) {
            ...
            var btnSelector = '#staffForm button[type="submit"], button[form="staffForm"]';
            var isEdit = !!self.currentStaffId;
            
            var options = {
                loadingText: isEdit ? TRANSLATION.general.updating : TRANSLATION.general.saving,
                normalText: isEdit ? TRANSLATION.general.update : TRANSLATION.general.save,
...
var StatsManager = Utils.createStatsManager({
    ...
    onError: TRANSLATION.general.failedToLoadStatistics
});
